  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div className="card fade-in" style={{ padding: '24px', marginBottom: '24px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: 800, marginBottom: '16px', color: 'var(--text-primary)' }}><i className="fas fa-user" style={{ marginRight: '8px', color: '#4f46e5' }}></i>Profile Information</h3>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            {['indigo', 'violet', 'rose', 'teal', 'amber'].map(c => (
              <button key={c} onClick={() => updatePrefs({ accent_color: c })} style={{ width: '36px', height: '36px', borderRadius: '50%', background: `var(--${c}, ${c === 'indigo' ? '#4f46e5' : c === 'violet' ? '#7c3aed' : c === 'rose' ? '#e11d48' : c === 'teal' ? '#0d9488' : '#d97706'})`, border: prefs.accent_color === c ? '3px solid white' : '2px solid transparent', boxShadow: prefs.accent_color === c ? '0 0 0 2px #4f46e5, 0 4px 12px rgba(79,70,229,0.3)' : 'none', cursor: 'pointer', transition: 'all 0.25s', transform: prefs.accent_color === c ? 'scale(1.15)' : 'scale(1)' }} aria-label={`Set ${c} accent`} />
            ))}
          </div>
        </div>
      </div>
